...
with t_graph:
    ...
    st.caption("Live interactive Cytoscape.js network graph connecting Store, Suppliers, Customer Udhaars, and Paytm Smart Micro-Loan.")
    ...
    <body style="margin:0; padding:0; background:transparent;">
        ...
        <script>
            var cy = cytoscape({
                ...
                elements: [
                    // Nodes
                    { data: { id: 'store', label: 'Namaste Kirana Store', type: 'store' } },
                    { data: { id: 'supplier', label: 'Delhi Grain Syndicate', type: 'supplier' } },
                    { data: { id: 'udhaar', label: 'Anil Dhaba', type: 'udhaar' } },
                    { data: { id: 'paytm', label: 'Paytm Smart Micro-Loan', type: 'paytm' } },
                    // Edges
                    { data: { source: 'store', target: 'supplier', label: 'OWES SUPPLIER' } },
                    { data: { source: 'udhaar', target: 'store', label: 'OWES UDHAAR' } },
                    { data: { source: 'paytm', target: 'store', label: 'BRIDGES DEFICIT' } }
                ],
                style: [
                    {
                        selector: 'node',
    ...
    components.html(cytoscape_html, height=400)
